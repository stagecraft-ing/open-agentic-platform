...
outputs:
  - .factory/pipeline-state.json (initialized)
  - .factory/adapter-manifest.yaml (resolved copy)
...
4. **Initialize pipeline state** — Create `.factory/pipeline-state.json` with status `running`, adapter identity, and all stages set to `pending`.
5. **Copy adapter manifest** — Write resolved manifest to `.factory/adapter-manifest.yaml` for downstream stages.
