"""Factory verification harness CLI.

Usage:
    python -m factory preflight --build-spec PATH --adapter PATH [--artifacts PATH]
    python -m factory gate --stage STAGE_ID --project PATH --checks-file PATH
    python -m factory feature --feature-id ID --type api|ui --project PATH --commands CMD [CMD...]
    python -m factory invariants --project PATH --adapter PATH
    python -m factory state init --adapter NAME --version VER --build-spec PATH --state PATH
    python -m factory state show --state PATH
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="factory", description="Factory verification harness")
