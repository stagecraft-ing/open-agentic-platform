...
// TypeScript types for the Elucid Pipeline panel.

/** Pipeline execution phase. */
export type ElucidPhase = 'idle' | 'process' | 'scaffolding' | 'complete' | 'failed';
...
/** Top-level pipeline state managed by ElucidPipelineContext. */
export interface ElucidPipelineState {
  runId: string | null;
  phase: ElucidPhase;
  stages: ProcessStage[];
...
export interface ElucidStepStartedEvent {
  runId: string;
  stepId: string;
  stepName: string;
}

export interface ElucidStepCompletedEvent {
  runId: string;
  stepId: string;
  artifacts: string[];
  tokenSpend: number;
...
export interface ElucidStepFailedEvent {
  runId: string;
  stepId: string;
  error: string;
  retryCount: number;
...
export interface ElucidGateReachedEvent {
  runId: string;
  stageId: string;
  stageName: string;
  gateType: 'checkpoint' | 'approval';
...
export interface ElucidScaffoldProgressEvent {
  runId: string;
  category: ScaffoldCategory;
  stepId: string;
  featureName: string;
...
export interface ElucidTokenUpdateEvent {
  runId: string;
  stageId: string;
  promptTokens: number;
  completionTokens: number;
...
export interface ElucidAgentOutputEvent {
  runId: string;
  stepId: string;
  line: string;
}
...
export function createInitialPipelineState(): ElucidPipelineState {
  return {
    runId: null,
    phase: 'idle',
