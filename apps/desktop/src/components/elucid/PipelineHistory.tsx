...
import type { PipelineRun, AuditEntry, ElucidPhase } from './types';
...
const PHASE_BADGE: Record<ElucidPhase, { label: string; className: string }> = {
...
const StatusBadge: React.FC<{ phase: ElucidPhase }> = ({ phase }) => {
...
export const PipelineHistory: React.FC<{ projectPath?: string }> = ({
  ...
  const { state, loadPipelineStatus } = useElucidPipeline();
  ...
  const fetchRuns = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await apiCall<PipelineRun[]>('list_elucid_runs', {
