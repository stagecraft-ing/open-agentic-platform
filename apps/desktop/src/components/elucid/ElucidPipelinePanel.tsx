...
// Top-level Elucid pipeline panel registered in TabContent.
...
import { ElucidPipelineProvider, useElucidPipeline } from './ElucidPipelineContext';
...
function ElucidPipelinePanelInner({ projectPath }: { projectPath?: string }) {
  ...
  return (
    <div className="h-full flex flex-col text-foreground relative">
      ...
      <header className="flex items-center gap-3 px-4 py-2.5 border-b border-border shrink-0">
        ...
        <h1 className="text-sm font-semibold flex-1">Elucid Pipeline</h1>
        {state.runId && (
          <span
...
export const ElucidPipelinePanel: React.FC<{ projectPath?: string }> = ({
  projectPath,
}) => {
  return (
    <ElucidPipelineProvider>
      <ElucidPipelinePanelInner projectPath={projectPath} />
    </ElucidPipelineProvider>
  );
};
