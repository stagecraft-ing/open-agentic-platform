...
static ELUCID_RUNS: LazyLock<Mutex<HashMap<String, ElucidRunState>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));

// ---------------------------------------------------------------------------
// Process stage constants  (the 6 Elucid pipeline stages)
// ---------------------------------------------------------------------------
...
/// Start a new Elucid pipeline run. Creates an in-memory run state and emits
/// `elucid:workflow_started`.
#[tauri::command]
pub async fn start_elucid_pipeline(
    ...
    .map_err(|e| format!("emit elucid:workflow_started failed: {e}"))?;
...
pub async fn get_elucid_pipeline_status(
    ...
    let runs = ELUCID_RUNS.lock().map_err(|e| e.to_string())?;
    runs.get(&run_id)
...
/// `elucid:step_started` for the next stage.
#[tauri::command]
pub async fn confirm_elucid_stage(
    ...
    let next_stage = {
        let mut runs = ELUCID_RUNS.lock().map_err(|e| e.to_string())?;
        let state = runs
            .get_mut(&run_id)
            .ok_or_else(|| format!("run not found: {run_id}"))?;
    ...
    if let Some(next_id) = next_stage {
        ...
        .map_err(|e| format!("emit elucid:step_started failed: {e}"))?;
    }

    Ok(())
}

/// Reject a gate stage. Records feedback and emits `elucid:stage_rejected`.
#[tauri::command]
pub async fn reject_elucid_stage(
    ...
    .map_err(|e| format!("emit elucid:stage_rejected failed: {e}"))?;

    Ok(())
}

/// List all Elucid pipeline runs for a project by scanning
/// `<project_path>/.elucid/runs/*/state.json`. Returns an empty vec when the
/// directory does not exist.
#[tauri::command]
pub async fn list_elucid_runs(
    project_path: String,
    ...
    let entries = std::fs::read_dir(&runs_dir)
        .map_err(|e| format!("read .elucid/runs failed: {e}"))?;
...
/// Mark a failed scaffold step as skipped and emit `elucid:step_skipped`.
#[tauri::command]
pub async fn skip_elucid_step(
    app: AppHandle,
    run_id: String,
    step_id: String,
) -> Result<(), String> {
    ...
    .map_err(|e| format!("emit elucid:step_skipped failed: {e}"))?;
