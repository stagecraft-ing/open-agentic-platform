...
Elucid solves these by separating concerns into three layers with formal contracts between them.
...
The current system invokes one agent for the entire application. Elucid invokes a **fresh agent per feature**:
...
Pipeline state is written to `.elucid/pipeline-state.json` after every successful step:
...
| Dimension | Current (the_factory + template) | Elucid |
|-----------|----------------------------------|--------|
...
| Audit trail | Missing in output | Preserved in .elucid/ directory |
