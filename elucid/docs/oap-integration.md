# Elucid + Open Agentic Platform: Integration Architecture

## The Match

Elucid and OAP solve complementary problems and share structural DNA:

| Concern | Elucid | OAP | Integration |
|---------|--------|-----|-------------|
| Workflow sequencing | Pipeline Orchestrator (stages 0-6) | `crates/orchestrator` (DAG manifest) | Elucid pipeline **becomes** an OAP workflow manifest |
| Artifact passing | `.elucid/` directory (build-spec, pipeline-state) | `$OAP_ARTIFACT_DIR/<run_id>/<step_id>/` | Elucid artifacts map 1:1 to OAP step artifacts |
| Agent invocation | Per-feature agent with ~1-3K token context | `dispatch_via_governed_claude()` — Claude CLI spawn | Each Elucid agent invocation = one OAP governed dispatch |
| Verification | Harness checks (schema validation, grep, commands) | Policy gates (checkpoint, approval, tier enforcement) | Elucid gates become OAP checkpoint gates |
| Human review | Stage handoff reports with confirm/reject | `GateHandler::await_checkpoint()` → UI dialog | Elucid confirmations surface in OPC desktop |
| Crash recovery | `.elucid/pipeline-state.json` | `WorkflowState` in `state.json` (FR-001) | OAP state subsumes Elucid state — single source of truth |
...
│                                                                       │
│  Orchestrator loads Elucid workflow manifest (DAG of stages)          │
...
Stagecraft is the organizational control plane. For Elucid integration it handles:

1. **Project registration** — user creates a project, selects "Elucid pipeline" as delivery method
...
### Elucid Pipeline as OAP Workflow

The Elucid pipeline translates directly to an OAP `WorkflowManifest`. The key insight: stages 1-5 are a fixed DAG, but stage 6 (scaffolding) fans out into dynamic sub-steps based on the Build Spec content.

#### Phase 1 Manifest: Process Stages (Fixed DAG)

```yaml
# .elucid/workflow-manifest.yaml — generated at pre-flight
steps:
  - id: "s0-preflight"
    agent: "elucid-preflight"
    effort: quick
    inputs:
...
steps:
  ...
  - id: "s6b-data-User"
    agent: "elucid-data-scaffolder-next-prisma"
    effort: quick
    inputs:
      - "s5-ui-specification/build-spec.yaml"
  ...
  - id: "s6c-api-list-sites"
    agent: "elucid-api-scaffolder-next-prisma"
    effort: quick
    inputs:
      - "s5-ui-specification/build-spec.yaml"
  ...
  - id: "s6d-ui-dashboard"
    agent: "elucid-ui-scaffolder-next-prisma"
    effort: quick
    inputs:
      - "s5-ui-specification/build-spec.yaml"
  ...
  - id: "s6e-configure"
    agent: "elucid-configurer-next-prisma"
    effort: quick
    inputs:
      - "s5-ui-specification/build-spec.yaml"
  ...
  - id: "s6f-trim"
    agent: "elucid-trimmer-next-prisma"
    effort: quick
    inputs:
...
#[tauri::command]
pub async fn generate_elucid_scaffold_manifest(
...
Each Elucid agent registers in OPC's `AgentDb` with appropriate permissions:

| Elucid Agent | OPC Agent Name | Permissions | Max Tier |
|-------------|---------------|-------------|----------|
| Business Requirements Analyst | `elucid-business-analyst` | file_read | Tier 1 |
| Service Designer | `elucid-service-designer` | file_read | Tier 1 |
| Data Architect | `elucid-data-architect` | file_read | Tier 1 |
| API Architect | `elucid-api-architect` | file_read | Tier 1 |
| UI Architect | `elucid-ui-architect` | file_read | Tier 1 |
| Data Scaffolder (per adapter) | `elucid-data-scaffolder-{adapter}` | file_read, file_write | Tier 2 |
| API Scaffolder (per adapter) | `elucid-api-scaffolder-{adapter}` | file_read, file_write | Tier 2 |
| UI Scaffolder (per adapter) | `elucid-ui-scaffolder-{adapter}` | file_read, file_write | Tier 2 |
| Configurer (per adapter) | `elucid-configurer-{adapter}` | file_read, file_write | Tier 2 |
| Trimmer (per adapter) | `elucid-trimmer-{adapter}` | file_read, file_write | Tier 2 |
| Validator | `elucid-validator` | file_read, network (for builds) | Tier 2 |
...
When OPC dispatches an Elucid agent, it builds the prompt from:

```
┌─────────────────────────────────────────────┐
│ 1. Elucid agent prompt (~50 lines)          │  ← from adapters/{name}/agents/{role}.md
...
### axiomregent Policy Rules for Elucid

axiomregent receives Elucid-specific policy shards:

```yaml
# Elucid scaffolding policy shard
tools:
...
User → Stagecraft UI:
  ...
  "Delivery method: Elucid pipeline"
  "Adapter: next-prisma"
...
```
OPC Desktop:
  User opens project → sees "Elucid Pipeline" tab
...
For each stage (1-5):
  ...
  OPC Orchestrator:
    ...
    → Resolve agent: "elucid-business-analyst"
  ...
  OPC Orchestrator:
    → Runs Elucid verification checks (schema validation, artifact existence)
...
1. **Elucid workflow type** — orchestrator recognizes "elucid" manifests and handles fan-out
2. **Dynamic manifest generation** — new command: `generate_elucid_scaffold_manifest(build_spec, adapter)`
3. **Post-step verification** — hook after scaffolding steps runs adapter `feature_verify` commands
4. **Elucid UI panel** — OPC desktop gets an "Elucid Pipeline" view with stage visualization
5. **Policy shard** — axiomregent loads Elucid-specific policy (allowed paths, commands)
6. **Stagecraft API** — endpoints for Elucid project init, stage confirmation, audit

### Shared

1. **Elucid adapter manifests ship as OAP skills** — each adapter's agents become `.claude/commands/` skill files
...
1. **No protocol mismatch** — Elucid's pipeline is already a DAG with artifacts; OAP's orchestrator expects exactly that
2. **Governance for free** — axiomregent already enforces file_write restrictions and command allowlists; Elucid just declares which paths and commands are legal per adapter
3. **Bounded context preserved** — OAP dispatches each step as a fresh Claude CLI session; Elucid's ~1-3K token agent prompts are exactly what the dispatch system expects
4. **Human checkpoints native** — OAP's `StepGateConfig::Checkpoint` is Elucid's "confirm before proceeding" pattern
5. **Crash recovery unified** — OAP's `WorkflowState` already tracks step completion and supports resume; no need for a separate `.elucid/pipeline-state.json`
