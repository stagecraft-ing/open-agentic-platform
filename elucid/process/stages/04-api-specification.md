...
outputs:
  - .elucid/build-spec.yaml (api section populated)
...
Populate the `api` section of `.elucid/build-spec.yaml`. Also populate `auth`, `project` (with variant), and `business_rules` sections at this stage since we now have all the information.
