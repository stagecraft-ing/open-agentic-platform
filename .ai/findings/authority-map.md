...
| Agent permissions | SQLite `agents` table: `enable_file_read`, `enable_file_write`, `enable_network` | Agent creation/edit UI shows toggles | **RESOLVED (035)** — flags enforced at runtime via per-session axiomregent subprocess. Residual: no-lease tool calls bypass checks (Risk 1, medium). |
| Safety tiers | `crates/agent/src/safety.rs` — Tier1/2/3 classification + `calculate_plan_tier()` | GovernanceSurface shows hardcoded tier labels | **MEDIUM** — tier system is consulted on every tool dispatch (035), but tier definitions are code-only, not spec-governed. |
...
- ~~**Agent permission authority is illusory.**~~ **RESOLVED (Feature 035).** All 7 `--dangerously-skip-permissions` sites replaced with governed dispatch. Permission flags enforced by per-session axiomregent subprocess via `OPC_GOVERNANCE_GRANTS` env. Residual: no-lease tool calls bypass checks (Risk 1).
