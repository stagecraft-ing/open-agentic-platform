...
| Governed tool execution | `axiomregent` MCP router (`crates/axiomregent/src/router/mod.rs`) — `gov.preflight`, `gov.drift`, `snapshot.*`, `workspace.*` | MCPManager shows sidecar status + probe port; GovernanceSurface shows safety tier labels | **HIGH** (downgraded from CRITICAL) — sidecar is now **live** (Feature 033) and visible in UI, but agent execution still bypasses it (`--dangerously-skip-permissions`). Tool dispatch through axiomregent is the next enforcement gap. |
...
- ~~axiomregent is dead~~ **RESOLVED (Feature 033).** `spawn_axiomregent()` is now called at `lib.rs:190`. Sidecar starts, probe port announced on stderr, UI surfaces status. However, agent execution still bypasses axiomregent — the governed dispatch path is live but unused for actual tool calls.
