...
// ---------------------------------------------------------------------------
// Known adapters (from elucid/adapters/)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Elucid pipeline stages (7-stage pipeline)
// ---------------------------------------------------------------------------
...
function compileDefaultRules(adapter: string, overrides?: PolicyOverrides) {
  ...
  // from the adapter manifest in elucid/adapters/<adapter>/adapter-manifest.md
...
async function getActivePipeline(projectId: string): Promise<PipelineRow> {
  ...
  const rows = await db
    .select()
    .from(elucidPipelines)
    .where(eq(elucidPipelines.projectId, projectId))
    .orderBy(desc(elucidPipelines.createdAt))
    .limit(1);

  if (rows.length === 0) {
    throw APIError.notFound("no Elucid pipeline found for this project");
  }
  return rows[0];
}
...
async function appendAudit(
  ...
  const [row] = await d
    ...
    .returning({ id: elucidAuditLog.id });
  return row.id;
}
...
export const initPipeline = api(
  { expose: true, method: "POST", path: "/api/projects/:id/elucid/init" },
  async (req: InitRequest): Promise<InitResponse> => {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (req.business_docs && req.business_docs.length > 0) {
        await tx.insert(elucidBusinessDocs).values(
      ...
      // Seed stage rows
      await tx.insert(elucidStages).values(
...
export const getStatus = api(
  { expose: true, method: "GET", path: "/api/projects/:id/elucid/status" },
  async (req: { id: string }): Promise<StatusResponse> => {
    ...
    const stageRows = await db
      .select()
      .from(elucidStages)
      .where(eq(elucidStages.pipelineId, pipeline.id))
      .orderBy(asc(elucidStages.stageId));
    ...
    const scaffoldRows = await db
      .select({
        promptTokens: elucidScaffoldFeatures.promptTokens,
        completionTokens: elucidScaffoldFeatures.completionTokens,
      })
      .from(elucidScaffoldFeatures)
      .where(eq(elucidScaffoldFeatures.pipelineId, pipeline.id));
    ...
    if (pipeline.policyBundleId) {
      const bundles = await db
        .select()
        .from(elucidPolicyBundles)
        .where(eq(elucidPolicyBundles.id, pipeline.policyBundleId))
        .limit(1);
      if (bundles.length > 0) {
...
export const confirmStage = api(
  {
    expose: true,
    method: "POST",
    path: "/api/projects/:id/elucid/stage/:stageId/confirm",
  },
  async (req: ConfirmRequest): Promise<ConfirmResponse> => {
    ...
    const stageRows = await db
      ...
      .where(
        and(
          eq(elucidStages.pipelineId, pipeline.id),
          eq(elucidStages.stageId, req.stageId)
        )
      )
      .limit(1);
    ...
    await db
      ...
      .where(eq(elucidStages.id, stage.id));
...
export const rejectStage = api(
  {
    expose: true,
    method: "POST",
    path: "/api/projects/:id/elucid/stage/:stageId/reject",
  },
...
export const getAudit = api(
  { expose: true, method: "GET", path: "/api/projects/:id/elucid/audit" },
  async (req: AuditRequest): Promise<AuditResponse> => {
    const pipeline = await getActivePipeline(req.id);
    const limit = Math.min(req.limit ?? 100, 500);

    const conditions = [eq(elucidAuditLog.pipelineId, pipeline.id)];
    if (req.from) {
      ...
      conditions.push(gte(elucidAuditLog.timestamp, parsed));
    }
    ...
    const entries = await db
      ...
      .orderBy(asc(elucidAuditLog.timestamp))
      .limit(limit);
...
export const triggerDeploy = api(
  { expose: true, method: "POST", path: "/api/projects/:id/elucid/deploy" },
...
export const reportTokenSpend = api(
  {
    expose: true,
    method: "POST",
    path: "/api/projects/:id/elucid/token-spend",
  },
  async (req: TokenSpendRequest): Promise<void> => {
    ...
    const [existing] = await db
      .select({ id: elucidAuditLog.id })
      .from(elucidAuditLog)
      .where(
        and(
          eq(elucidAuditLog.pipelineId, pipeline.id),
          eq(elucidAuditLog.event, "token_spend_reported"),
          eq(elucidAuditLog.stageId, req.stage_id),
          sql`${elucidAuditLog.details}->>'run_id' = ${req.run_id}`
        )
      )
      .limit(1);
    ...
    const stageRows = await db
      ...
      .where(
        and(
          eq(elucidStages.pipelineId, pipeline.id),
          eq(elucidStages.stageId, req.stage_id)
        )
      )
      .limit(1);
    ...
    await db
      .update(elucidStages)
      .set({
        promptTokens: sql`${elucidStages.promptTokens} + ${req.prompt_tokens}`,
        completionTokens: sql`${elucidStages.completionTokens} + ${req.completion_tokens}`,
        model: req.model,
      })
      .where(eq(elucidStages.id, stageRows[0].id));
