...
// Adapter-specific scopes compiled from factory/adapters/<name>/manifest.yaml.
// Generated by scripts/compile-adapter-scopes.js → build/adapter-scopes.json
// then copied here. Re-run the script after updating any adapter manifest.
const _adapterScopesPath = join(dirname(fileURLToPath(import.meta.url)), "adapter-scopes.json");
const _adapterScopesData = JSON.parse(readFileSync(_adapterScopesPath, "utf8")) as {
  compiled_at: string;
  adapters: Record<string, { file_write_scope: string[]; allowed_commands: string[] }>;
};
const ADAPTER_SCOPES: Record<string, { file_write_scope: string[]; allowed_commands: string[] }> =
  _adapterScopesData.adapters ?? {};
