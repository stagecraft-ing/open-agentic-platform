-- Spec 141 §2.2 — align acme-vue-node `scaffold_source_id` with the
-- upstream's self-declared `template.json::templateName` ("acme-vue-node"),
-- amending spec 140 §2.1 which fixed the literal as
-- `acme-vue-node-template`.
--
-- Two effects, both idempotent:
--
--   1. UPDATE the migration-36 synthetic substrate row(s) to carry
--      `acme-vue-node` (instead of `acme-vue-node-template`) in
...
-- Migration 36 continues to insert `acme-vue-node-template`; migration
-- 37 forward-migrates the value to `acme-vue-node` in the same
...
UPDATE factory_artifact_substrate
 ...
 WHERE origin = 'oap-self'
   ...
   AND frontmatter->>'scaffold_source_id' = 'acme-vue-node-template';
-- `content_hash` is intentionally not updated. Migration 36 chose a
-- marker-style sha256 of `spec-140-migration-36-acme-vue-node-v1`
...
SELECT
    ...
    'artifact.synced',
    NULL,
    jsonb_build_object('scaffold_source_id', 'acme-vue-node-template'),
    jsonb_build_object(
        'scaffold_source_id', 'acme-vue-node',
