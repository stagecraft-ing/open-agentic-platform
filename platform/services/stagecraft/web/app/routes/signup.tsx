...
import { authSignup } from "../lib/auth-api.server";
...
export async function action({ request }: { request: Request }) {
  ...
  const res = await authSignup(request, email, name, password);
  ...
  return (
    <div className="min-h-full container px-4 mx-auto my-16 max-w-sm">
      ...
      <p className="mt-4 text-sm text-gray-600 dark:text-gray-400">
        ...
        <Link to="/signin" className="text-indigo-600 hover:text-indigo-500 dark:text-indigo-400">
          Sign in
        </Link>
      </p>
    </div>
