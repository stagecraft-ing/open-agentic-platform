...
- `scaffold/` — the absorbed scaffold subflow:
  ...
  - `moduleCatalog.ts` (pure derivation + helpers): `deriveModuleCatalog(manifests)`, `deriveInstallOrder(catalog)`, `PROFILE_MODULES`, `PRESETS`, `pickProfileFromModules(variant, modules)`, `extrasFor(catalog, profile, selected)`, `isKnownModule(catalog, id)`, `isModuleManifestPath(path)`, plus the `RawModuleManifest`/`ModuleDescriptor` types. Spec 227 Stage 1: the catalog derives at request time from the adapter's module `manifest.json` rows in the substrate (no hand-mirrored `MODULE_CATALOG`/`INSTALL_ORDER`); the Encore-side loader plus `GET /api/factory/module-catalog` live in `api/factory/moduleCatalog.ts`, behind a short-TTL per-org cache (`moduleCatalogCache.ts`). Profiles select AUTH_DRIVER and ship no modules by default (spec 199 FR-007 cutover, 2026-06-11; the template-distributor shape and `detectProfile` are retired).
