...
- `validate_spec_id_resolution(cert, repo_root)` (OAP `governance_certificate.rs:1397-1426`) loads the registry and looks up the cert's `intent.spec_id`. It calls exactly two functions: `open_agentic_spec_registry_reader::load(&registry_path)` (`:1410`) and `registry.find_by_id(spec_id)` (`:1419`).
- These are the public consumer-API of the typed reader, not compiler internals. The crate `open_agentic_spec_registry_reader` is the library face of `registry-consumer` (OAP `tools/spec-spine/registry-consumer/Cargo.toml`, lib name `open_agentic_spec_registry_reader`; `src/lib.rs:472` `pub fn load`, `:526` `pub fn find_by_id`). The cert code never touches `SHAPE_TABLE`, `KNOWN_KEYS`, or any compiler-side type. Those are spec-compiler internals and do not appear in the cert module at all.
...
The dependency is declared with that single purpose: "Cut D W-10 (spec 102 G-2 follow-up) - validate StageRecord.spec_id resolves against the spec spine via the typed-reader library. `open_agentic_spec_registry_reader = { path = "../../tools/spec-spine/registry-consumer" }`" (OAP `crates/factory-engine/Cargo.toml:18-20`).
