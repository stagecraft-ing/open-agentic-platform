...
The authoritative crate, package, and tool inventory lives in [`.derived/codebase-index/CODEBASE-INDEX.md`](../.derived/codebase-index/CODEBASE-INDEX.md), auto-generated by `codebase-indexer` from the manifests themselves. Each entry includes its owning spec via `[package.metadata.oap].spec` (Rust) or `oap.spec` (npm), so the spec spine is the single source for "what is this crate."
...
2. **Compile** — `spec-compiler compile` produces `.derived/spec-registry/registry.json`
