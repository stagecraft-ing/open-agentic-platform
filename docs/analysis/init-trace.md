...
| `apps/` (directory listing) | App targets root. | `ls apps/` (`init.md:34`). | OAP-specific | high | `apps/opc/` is OAP's OPC. The path convention is generic, but a spec-spine adopter may not have an `apps/` dir at all. | The convention "applications go in `apps/`" is generic. The presence and content are OAP-specific. |
