...
|---|---|---|---|---|
| `apps/opc` | `@opc/desktop` | 0.3.2 | 3 | `032-opc-inspect-governance-wiring-mvp` |
...
| `apps/opc/package.json` | `@opc/claude-code-bridge: workspace:*` |
| `apps/opc/package.json` | `@opc/ui: workspace:*` |
| `apps/opc/package.json` | `@opc/workspace-sdk: workspace:*` |
...
`git grep -nE "from\s+['\"]\.\.?/.+/apps/opc/" -- '*.ts' '*.tsx' '*.js' '*.jsx' '*.mjs'` returns **zero results**.

Same observation — `apps/opc` does not import other apps via path (there are no other apps), and other packages do not reach into `apps/opc/` via path.
...
Path-literal references to `apps/opc/` and `packages/` that need updates in I7 alongside the move:

| file:line | category | update target |
|---|---|---|
| `apps/opc/src-tauri/src/commands/claude.rs:154,158,161,1200` | path-literal | `packages/provider-registry/dist/node-sidecar.js` → `product/packages/provider-registry/dist/node-sidecar.js` |
| `apps/opc/vite.config.ts:18` | path-literal (comment) | `packages/ui/src` → `product/packages/ui/src` (comment update; vite resolves via tsconfig paths, not literal) |
...
- **Apps to move:** 1 (`apps/opc`) → `product/apps/opc`
...
- **`apps/opc/src-tauri/Cargo.toml` path deps deepen by one level** after I7 (`../../../crates/...` → `../../../../crates/...`). 13 path-dep declarations affected; reformulated as workspace deps in I1 eliminates the literal paths and removes the need to update at I7 time.
- **Tauri sidecar runtime path** (`apps/opc/src-tauri/src/commands/claude.rs:154-161,1200`) needs the new `product/packages/provider-registry/dist/node-sidecar.js` path — single hot spot to update inside the moving tree.
...
2. **`apps/opc/src-tauri/` Cargo workspace** — see D2 Open Question 1 (SQLite isolation). I7 changes path-dep depths regardless; coupled with I1's decision.
...
4. **TypeScript paths config** — `apps/opc/tsconfig.json` and similar may carry `paths:` mappings to `packages/...`. Not surveyed in this phase; verify in I7 atomic-move PR before declaring complete.
