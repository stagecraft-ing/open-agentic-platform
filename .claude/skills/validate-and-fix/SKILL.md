...
- **`ci-fast-tools`**: parallel xargs fan-out across tool manifests with shared `CARGO_TARGET_DIR`; `cargo test -- --list` post-pass asserting each `CI_REGISTRY_CONSUMER_CONTRACTS` prefix has ≥1 match; `spec-lint --fail-on-warn` smoke; `codebase-indexer compile` smoke (broad staleness gate retired per spec 188 Phase 4b). Mirrors `spec-conformance.yml`.
