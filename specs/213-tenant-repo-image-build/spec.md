...
step. The upstream template (template-encore) ships a complete
...
- template-encore carries `.github/workflows/encore-cd.yml.example`:
...
`encore-cd.yml.example` in template-encore remains untouched upstream;
...
2. **Workflow token scope**: confirm `GITHUB_TOKEN` with `packages:
   ...
   (GovAlta-EMU style orgs may restrict package creation; if so, fall back
