...
language: en
summary: >
  Ingest the Elucid software factory framework into OAP as a first-class citizen,
...
code_aliases: ["ELUCID_INGESTION", "ELUCID_CONTRACTS"]
---

# Feature Specification: Elucid Ingestion as First-Class Delivery Engine
...
Elucid is a modular, tech-agnostic software factory framework that separates the *process* of building software (requirements, design, specification) from *implementation* details (frameworks, languages, patterns). Its 7-stage pipeline, formal contract schemas, and pluggable adapter system map 1:1 onto OAP's orchestrator, policy kernel, and agent dispatch.

This spec defines how Elucid enters the OAP repository, how its contract schemas become Rust types, and how its adapters register with the platform.

## Scope

### In scope

- Git subtree ingestion of Elucid into `elucid/` at repository root
- New Rust crate `crates/elucid-contracts/` with typed representations of all four Elucid contract schemas (Build Spec, Adapter Manifest, Pipeline State, Verification Contract)
- Adapter discovery and registration mechanism
- Process agent prompt loading from `elucid/process/agents/`
- Contract schema validation (YAML/JSON → typed Rust structs with serde)
- Integration with spec-compiler to register Elucid artifacts in the spec registry
...
Elucid source from `analyticus/elucid` SHALL be ingested via `git subtree add` into `elucid/` at repository root. The subtree prefix preserves Elucid's internal structure:
...
A new crate `crates/elucid-contracts/` SHALL provide Rust types for all four Elucid contract schemas:
...
An `AdapterRegistry` SHALL discover and load all adapter manifests from `elucid/adapters/*/manifest.yaml`:
...
impl AdapterRegistry {
    pub fn discover(elucid_root: &Path) -> Result<Self, DiscoveryError>;
...
A `ProcessAgentLoader` SHALL read agent prompt files from `elucid/process/agents/` and adapter-specific agents from `elucid/adapters/{name}/agents/`:
...
pub fn load_process_agents(elucid_root: &Path) -> Result<Vec<AgentPrompt>, LoadError>;
...
The spec-compiler SHALL be extended to recognize Elucid Build Specs as compilable artifacts. When a project has a frozen Build Spec at `.elucid/build-spec.yaml`, the compiler SHALL:
- Index it in `build/spec-registry/registry.json` under a new `elucid_projects` section
...
Elucid subtree updates SHALL NOT require changes to OAP code outside `crates/elucid-contracts/`. The contract crate is the only coupling point.

**NF-003: Adapter Extensibility**
Adding a new adapter SHALL require only creating a new directory under `elucid/adapters/{name}/` with a valid `manifest.yaml`. No code changes to OAP.
...
```bash
git subtree add --prefix=elucid \
  git@github.com:analyticus/elucid.git main \
  --squash -m "feat(074): ingest Elucid as first-class delivery engine"
```
...
1. Create `crates/elucid-contracts/Cargo.toml` with dependencies
...
7. Write tests: round-trip parse of `elucid/contract/examples/*.yaml`

### Phase 3: Registry & Loaders (2 days)

1. Implement `AdapterRegistry::discover()` — glob `elucid/adapters/*/manifest.yaml`
...
1. Add `elucid_projects` section to registry schema
2. Detect `.elucid/build-spec.yaml` during compilation
3. Index project name, adapter, hash, stage status

## Success Criteria

- **SC-001**: All four Elucid contract examples parse without error into Rust types
...
- **SC-005**: `cargo build --release` succeeds for `crates/elucid-contracts/`

## Dependencies

| Spec | Relationship |
|------|-------------|
| 075-elucid-workflow-engine | Consumes contract types for manifest generation |
| 076-elucid-desktop-panel | Consumes contract types for UI rendering |
| 077-stagecraft-elucid-api | Consumes adapter registry for project init |
| 067-tool-registry | Elucid agents register as tool-registry entries |
...
| Elucid subtree update breaks contract types | NF-002: contract crate is sole coupling point; version-pin subtree |
| Large subtree bloats repo | Elucid is ~2MB total (mostly markdown + YAML); negligible |
