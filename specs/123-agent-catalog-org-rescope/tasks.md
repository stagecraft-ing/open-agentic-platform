...
- [ ] **T072** [P] Update `product/apps/opc/src-tauri/src/commands/stagecraft_client.rs` and `sync_client.rs` for the v2 envelope wire format. Compile-time `const` for `AGENT_CATALOG_ENVELOPE_VERSION = 2` and `PROJECT_AGENT_BINDING_ENVELOPE_VERSION = 1`.
...
- [ ] **T094** [P] Run the grep gate from spec A-5: `grep -rn "agent_catalog\.project_id\|agent_catalog_audit\.project_id\|agent_policies\.project_id\|agentCatalog\.projectId" platform/services/stagecraft crates product/apps/opc`. Must return zero hits outside historical migration files (27, 28), this spec's migration script (30), frozen superseded specs, and this spec's body.
