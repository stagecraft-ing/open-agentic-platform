...
- **FU-002 — deployd-api dual-writer fix (same shape as L-003).**
  *Closed 2026-05-11 — narrow setup.sh edit retired the dual
  ...
  edit mirroring L-003's stagecraft shape (replace helm-upgrade
...
- **FU-024 — Chart-side L-003 mirror for deployd-api
  ...
  *Stub-templating revalidation, performed at filing time
  ...
  Templated from L-003's stagecraft fix
  (`charts/stagecraft/values-hetzner.yaml:1-6` +
  `charts/stagecraft/templates/deployment.yaml:33`); the
  ...
  - `charts/deployd-api/values-hetzner.yaml:1-3` —
    ...
    with no `pullPolicy`. L-003 stagecraft equivalent
  ...
  - `charts/deployd-api/templates/deployment.yaml:32` —
    ...
    stagecraft fix, where a pre-existing `imagePullPolicy`
  ...
  *Done when (revalidation FIRST, per the discipline that
  ...
  (b) **Drop `tag: latest` from
      `charts/deployd-api/values-hetzner.yaml:1-3`,**
      replacing with the L-003 stagecraft pattern:
...
- **(a) Server.** `platform/services/stagecraft/api/auth/refreshCookie.ts`
...
- **(b) Client.** `platform/services/stagecraft/web/app/lib/fetchWithRefresh.ts`
  ...
  *Fix-time disposition pinned: per-site wrapper, not inline-in-callAPI.*
  ...
  (`platform/services/stagecraft/package.json:gen`,
...
- **(c) Regression tests.** Three new vitest files, 21 tests
  total, all passing in the full stagecraft suite (45 files /
  426 tests):
...
- *Approach.* Stub Option 1 ("shortened lifetime, real
  ...
  `stagecraft-server` client's `access_token_lifetime` from
  the 1800s default to 60s via
  `PUT /auth/v1/clients/stagecraft-server` carrying the
...
- *Server-side evidence chain* (from
  ...
  stagecraft-system deploy/stagecraft-api --tail=0 -f`):
...
- *Restore.* PUT the full snapshot JSON back to
  `/auth/v1/clients/stagecraft-server`. Verify reads
...
verbatim with the L-003 stagecraft mirror —
...
stagecraft block at setup.sh:362-374 in shape and tone.
...
- Before: two writers competed on the deployd-api Helm
  ...
  L-003 stagecraft incident on 2026-05-08).
...
stagecraft — removing `tag: latest` from
...
stagecraft fix did ship the chart-side hardening (see
`platform/charts/stagecraft/values-hetzner.yaml:1-6` and
`platform/charts/stagecraft/templates/deployment.yaml:33`);
...
$ helm template platform/charts/deployd-api \
    ...
    | grep -E "(image:|imagePullPolicy:)"
          image: "ghcr.io/stagecraft-ing/open-agentic-platform/deployd-api:dev"
