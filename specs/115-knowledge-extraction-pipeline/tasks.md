...
- [ ] **T041** Expose the resolver to stagecraft. Either: (a) HTTP shim in stagecraft hitting a small Rust binary, or (b) static JSON snapshot read by stagecraft directly from `.derived/policy/`. Decision: ship (b) for MVP — the policy compiler emits `.derived/policy/workspaces/{workspaceId}.json`; stagecraft reads with a 30s in-memory cache. Update `extractionPolicy.ts` from Phase 0 to source from this file with the `DEFAULT_DETERMINISTIC_ONLY_POLICY` fallback when the file is missing.
