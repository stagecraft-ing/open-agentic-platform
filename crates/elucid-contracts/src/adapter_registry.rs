...
//!
//! Discovers adapters by scanning `elucid/adapters/*/manifest.yaml` and
...
/// Registry of discovered Elucid adapters.
#[derive(Debug, Clone)]
...
impl AdapterRegistry {
    /// Discover all adapters under `elucid_root/adapters/*/manifest.yaml`.
    pub fn discover(elucid_root: &Path) -> Result<Self, DiscoveryError> {
        if !elucid_root.exists() {
            return Err(DiscoveryError::RootNotFound(elucid_root.to_path_buf()));
        }

        let adapters_dir = elucid_root.join("adapters");
...
mod tests {
    ...
    fn test_discover_real_adapters() {
        let elucid_root = std::path::Path::new("../../elucid");
        if !elucid_root.exists() {
            return;
        }
        let registry = AdapterRegistry::discover(elucid_root)
    ...
    fn test_capabilities_match_real_example() {
        let elucid_root = std::path::Path::new("../../elucid");
        let spec_path = elucid_root.join("contract/examples/cfs-womens-shelter.build-spec.yaml");
        if !spec_path.exists() {
            return;
        }
        let registry = AdapterRegistry::discover(elucid_root)
