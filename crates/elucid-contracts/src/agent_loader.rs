...
//!
//! Reads agent prompt files from `elucid/process/agents/` (process agents,
//! Tier 1 read-only) and `elucid/adapters/{name}/agents/` (scaffold agents,
//! Tier 2 read-write).
...
/// Load all process agents from `elucid_root/process/agents/`.
///
...
pub fn load_process_agents(elucid_root: &Path) -> Result<Vec<AgentPrompt>, LoadError> {
    let agents_dir = elucid_root.join("process").join("agents");
