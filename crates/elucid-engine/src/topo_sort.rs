...
// Spec: specs/075-elucid-workflow-engine/spec.md — FR-004

//! Entity dependency ordering via topological sort.
//!
//! Elucid entities can reference each other (e.g. `Site` references `Organization`).
//! Scaffolding steps must generate referenced entities before referencing ones.

use crate::ElucidError;
use elucid_contracts::build_spec::Entity;
...
pub fn topological_sort_entities(entities: &[Entity]) -> Result<Vec<String>, ElucidError> {
    ...
    if order.len() != n {
        ...
        let in_cycle: Vec<&str> = (0..n)
            ...
            .collect();
        return Err(ElucidError::CircularReference {
            cycle: in_cycle.join(" <-> "),
        });
    }
...
mod tests {
    use super::*;
    use elucid_contracts::build_spec::{Entity, Field, FieldType};
    ...
    fn circular_reference_detected() {
        ...
        assert!(matches!(result, Err(ElucidError::CircularReference { .. })));
    }
