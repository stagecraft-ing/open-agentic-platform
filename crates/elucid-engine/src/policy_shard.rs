...
// Spec: specs/075-elucid-workflow-engine/spec.md — FR-008

//! Elucid-specific policy shard generation.
//!
...
/// Generate a policy shard for an Elucid pipeline (FR-008).
///
/// The shard:
...
pub fn generate_elucid_policy_shard(
    ...
    let scope = format!("elucid:{}", adapter.adapter.name);
    ...
    rules.push(PolicyRule {
        ...
        scope: scope.clone(),
        gate: None,
        source_path: "elucid-engine/policy_shard".into(),
    ...
    rules.push(PolicyRule {
        ...
        description: "Block dangerous shell patterns in Elucid pipeline".into(),
        mode: "enforce".into(),
        scope: scope.clone(),
        gate: None,
        source_path: "elucid-engine/policy_shard".into(),
...
mod tests {
    ...
    fn minimal_build_spec() -> BuildSpec {
        use elucid_contracts::build_spec::*;
    ...
    fn policy_shard_contains_adapter_scope() {
        ...
        let bundle = generate_elucid_policy_shard(&adapter, &spec);

        assert!(bundle.shards.contains_key("elucid:test-adapter"));
        let rules = &bundle.shards["elucid:test-adapter"];
        assert!(rules.len() >= 2);
    }
