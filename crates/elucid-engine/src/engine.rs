...
//! Two-phase Elucid execution engine.
//!
//! Orchestrates:
...
//! and artifact management. Adds Elucid-specific verification, retry logic,
//! and pipeline state tracking.

use crate::agent_bridge::ElucidAgentBridge;
use crate::manifest_gen::{generate_process_manifest, generate_scaffold_manifest};
use crate::pipeline_state::{ElucidPipelineState, FailedFeature};
use crate::policy_shard::generate_elucid_policy_shard;
use crate::verify_harness::run_elucid_gate_check;
use crate::ElucidError;
use elucid_contracts::build_spec::BuildSpec;
use elucid_contracts::AdapterRegistry;
use orchestrator::manifest::WorkflowManifest;
use sha2::{Digest, Sha256};
use std::path::{Path, PathBuf};
use uuid::Uuid;

/// Configuration for the Elucid engine.
#[derive(Clone, Debug)]
pub struct ElucidEngineConfig {
    /// Path to the Elucid subtree (e.g., `elucid/`).
    pub elucid_root: PathBuf,
...
impl Default for ElucidEngineConfig {
    fn default() -> Self {
        Self {
            elucid_root: PathBuf::from("elucid"),
...
/// The Elucid two-phase workflow engine.
///
/// Coordinates process stages, Build Spec freeze, and scaffolding fan-out
/// using OAP orchestrator primitives.
pub struct ElucidEngine {
    config: ElucidEngineConfig,
    adapter_registry: AdapterRegistry,
}

impl ElucidEngine {
    /// Create a new engine with adapter discovery.
    pub fn new(config: ElucidEngineConfig) -> Result<Self, ElucidError> {
        let adapter_registry =
            AdapterRegistry::discover(&config.elucid_root).map_err(|e| {
                ElucidError::AdapterNotFound {
    ...
    /// Start a new Elucid pipeline.
    ///
    /// Returns the `run_id` and the initial `ElucidPipelineState`.
    ///
    ...
    pub fn start_pipeline(
    ...
    ) -> Result<PipelineStartResult, ElucidError> {
        let adapter = self
            .adapter_registry
            .get(adapter_name)
            .ok_or_else(|| ElucidError::AdapterNotFound {
        ...
        let phase1_manifest = generate_process_manifest(
            adapter,
            business_doc_paths,
            &self.config.elucid_root,
        )?;

        // Create agent bridge.
        let process_agents = elucid_contracts::agent_loader::load_process_agents(
            &self.config.elucid_root,
        )
        .unwrap_or_default();
        let adapter_path = self
            ...
            .join(adapter_name);
        let adapter_agents =
            elucid_contracts::agent_loader::load_adapter_agents(&adapter_path)
                .unwrap_or_default();
        let agent_bridge = ElucidAgentBridge::new(process_agents, adapter_agents);

        // Create initial pipeline state.
        let pipeline_state = ElucidPipelineState::new(run_id.to_string(), adapter_name);
    ...
    pub fn transition_to_scaffolding(
        ...
        pipeline_state: &mut ElucidPipelineState,
        ...
        let _policy_bundle = generate_elucid_policy_shard(adapter, &build_spec);
    ...
    pub async fn run_gate_checks(
        &self,
        stage_id: &str,
    ) -> Result<crate::verify_harness::GateCheckResult, ElucidError> {
        run_elucid_gate_check(
            stage_id,
    ...
    pub fn config(&self) -> &ElucidEngineConfig {
        &self.config
    }
...
pub struct PipelineStartResult {
    ...
    pub agent_bridge: ElucidAgentBridge,
    /// Initial pipeline state.
    pub pipeline_state: ElucidPipelineState,
}
...
mod tests {
    ...
    fn record_completion_updates_state() {
        let mut state = ElucidPipelineState::new("test", "adapter");
