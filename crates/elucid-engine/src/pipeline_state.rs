...
// Spec: specs/075-elucid-workflow-engine/spec.md — FR-009

//! Elucid-specific pipeline state maintained alongside the orchestrator's `WorkflowState`.

use serde::{Deserialize, Serialize};

/// Current phase of an Elucid pipeline.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum ElucidPhase {
...
/// Elucid-specific state tracked alongside the generic `WorkflowState` (FR-009).
...
impl ElucidPipelineState {
    ...
    pub fn transition_to_scaffolding(&mut self, build_spec_hash: String) {
        ...
        self.phase = ElucidPhase::Scaffolding;
        self.scaffolding = Some(ScaffoldingProgress::default());
    }

    /// Mark pipeline as complete.
    pub fn mark_complete(&mut self) {
        self.phase = ElucidPhase::Complete;
    }

    /// Mark pipeline as failed.
    pub fn mark_failed(&mut self) {
        self.phase = ElucidPhase::Failed;
    }
...
mod tests {
    ...
    #[test]
    fn initial_state() {
        let state = ElucidPipelineState::new("run-123", "next-prisma");
        assert_eq!(state.phase, ElucidPhase::Process);
    ...
    fn phase_transitions() {
        let mut state = ElucidPipelineState::new("run-123", "next-prisma");

        state.transition_to_scaffolding("abc123def".into());
        assert_eq!(state.phase, ElucidPhase::Scaffolding);
        ...
        state.mark_complete();
        assert_eq!(state.phase, ElucidPhase::Complete);
    }
    ...
    fn round_trip_serialization() {
        let mut state = ElucidPipelineState::new("run-456", "rust-axum");
        state.transition_to_scaffolding("hash".into());
        state.entity_completed("Widget");
        state.add_tokens(5000);

        let json = serde_json::to_string(&state).unwrap();
        let restored: ElucidPipelineState = serde_json::from_str(&json).unwrap();
